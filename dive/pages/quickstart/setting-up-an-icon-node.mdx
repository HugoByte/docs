...
Before you proceed, make sure the Kurtosis Engine is running in the background. If it's not already running, start it with the following command:
...
Executing above command will generate 'services.json,' in working directory which contains service details for the 'icon' chain.
...
- `--id`: Assign a custom chain ID for your ICON chain using the `--id` flag. This ID is used to uniquely identify your custom chain.

<Callout type="info" emoji="💡">
  *Tip:* If you want to start your icon chain for BTP support, you can also decentralize by adding the `--decentralisation` flag.
</Callout>

### Decentralize Already Running ICON Chain for BTP Support
